...
import { Menu, X, ScanLine, UserPlus, LogIn, LogOut, User, LayoutDashboard } from "lucide-react";
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {user && (
              <>
                <Link to="/mes-biens" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  location.pathname === "/mes-biens" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                }`}>Mes appareils</Link>
              </>
            )}
          </div>
            ...
            {user ? (
              <>
                <Button variant="ghost" asChild>
                  <Link to="/tableau-de-bord"><LayoutDashboard className="h-4 w-4 mr-1" /> Tableau de bord</Link>
                </Button>
                <Button variant="ghost" asChild>
                  <Link to="/profil"><User className="h-4 w-4 mr-1" /> {profile?.prenoms || "Profil"}</Link>
                </Button>
        ...
        {isOpen && (
          <div className="md:hidden pb-4 border-t border-border mt-2 pt-4 space-y-2">
            ...
            {user && (
              <Link to="/mes-biens" onClick={() => setIsOpen(false)} className="block px-4 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted">
                Mes appareils
              </Link>
            )}
              ...
              {user ? (
                <>
                  <Button variant="outline" asChild className="w-full">
                    <Link to="/tableau-de-bord" onClick={() => setIsOpen(false)}><LayoutDashboard className="h-4 w-4 mr-1" /> Tableau de bord</Link>
                  </Button>
