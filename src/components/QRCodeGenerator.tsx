...
const QRCodeGenerator = ({ token, bienNom }: QRCodeGeneratorProps) => {
  ...
  // Use the actual app origin so QR codes work in any environment
  const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://safetrace.lovable.app";
  ...
  const downloadPDF = async () => {
    ...
    ctx.fillText("Vérifier sur SafeTrace", pdfWidth / 2, size + 60);
      ...
      <CardContent className="space-y-4">
        {qrDataUrl && (
          <div className="flex justify-center">
            <div className="bg-card border-2 rounded-xl p-4 inline-block">
              ...
              <p className="text-center text-xs text-muted-foreground mt-2">{token}</p>
            </div>
          </div>
        )}
        <div className="space-y-1.5">
          ...
          <Select value={String(size)} onValueChange={(v) => setSize(Number(v))}>
            ...
            <SelectContent>
              {sizes.map((s) => <SelectItem key={s.value} value={String(s.value)}>{s.label}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={downloadPNG} className="w-full"><Download className="h-4 w-4 mr-1" /> PNG</Button>
          <Button variant="outline" onClick={downloadPDF} className="w-full"><FileText className="h-4 w-4 mr-1" /> Étiquette</Button>
        </div>
        <p className="text-xs text-muted-foreground text-center">Collez ce QR code sur votre bien. Tout scan notifiera votre compte.</p>
      </CardContent>
    </Card>
  );
};
