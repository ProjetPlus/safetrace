...
const Dashboard = () => {
  ...
  // Placeholder data - will be connected to Supabase
  const biens = [
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
              ...
              <p className="text-muted-foreground">Gérez et protégez vos biens</p>
            </div>
            <div className="flex gap-2">
              ...
              <Button variant="outline" asChild>
                <Link to="/mes-biens"><List className="h-4 w-4 mr-2" /> Tous mes biens</Link>
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { icon: Package, label: "Biens enregistrés", value: String(biens.length), color: "text-primary" },
              { icon: AlertTriangle, label: "Signalements actifs", value: "0", color: "text-destructive" },
              { icon: QrCode, label: "QR Codes générés", value: String(biens.length), color: "text-safe-green" },
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="font-display text-xl">Mes biens</CardTitle>
              ...
              {biens.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <Button asChild className="mt-4 bg-safe-green hover:bg-safe-green/90 text-white">
                    <Link to="/enregistrer-bien">Enregistrer mon premier bien</Link>
                  </Button>
                </div>
              ) : (
                  ...
                  {biens.map((bien) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-medium">{bien.nom}</div>
                          <div className="text-sm text-muted-foreground">{bien.type} · {bien.identifiant}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setQrDialog({ open: true, token: bien.token, nom: bien.nom })}
                          title="Voir QR Code"
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setTransferDialog({ open: true, bienNom: bien.nom })}
                          title="Transférer"
                        >
